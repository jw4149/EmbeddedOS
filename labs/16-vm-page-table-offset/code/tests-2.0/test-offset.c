...
static void fault_handler(regs_t *r) {
    ...
    domain_access_ctrl_set(DOM_client << dom_kern*2 | DOM_client << dom_heap*2);
...
static void branch_fault_handler(regs_t *r) {
    uint32_t fault_addr;
    uint32_t dfsr;
    ...
    // staff_domain_access_ctrl_set(DOM_client << dom_kern*2 | DOM_client << dom_heap*2);
...
void notmain(void) { 
    ...
    // vm_map_sec(pt, OneMB, OneMB, heap);
    for (int i = 0; i < OneMB; i++) {
        vm_pte_t * pte = vm_map_sec(pt, OneMB + i, OneMB + i, heap);
        trace("i = %d, va read out is: %d\n", i, pte->sec_base_addr);
    }
    ...
    // full_except_set_prefetch(branch_fault_handler);
