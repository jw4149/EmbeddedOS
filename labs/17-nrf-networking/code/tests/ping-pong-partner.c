...
enum { ntrial = 1000, timeout_usec = 1000000000, nbytes = 4 };
...
ping_pong_ack(nrf_t *s, nrf_t *c, int verbose_p) {
    ...
    uint32_t exp = 1234, got = 0;

    trace("starting ping pong.\n");

    for(unsigned i = 0; i < ntrial; i++) {
        trace("received %d packets [timeouts=%d]\n", 
               npackets, ntimeout);
        ...
        if (!net_get32(s, &got)) {
            ...
            nrf_output("received: %d\n", got);
            npackets++;
        }
        
        net_put32(s, client_addr, 4567);
...
void notmain(void) {
    ...
    nrf_t *s = server_mk_ack(0xe3e3e3, nbytes);
    nrf_t *c = client_mk_ack(0xe1e1e1, nbytes);
